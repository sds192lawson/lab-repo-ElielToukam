...
hmda_ms_2024 |>
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, MIssissippi, 2024",
       ...
       y = "Count of Loans") +
  theme_minimal()
##TA Feedback: This graph looks great except for one small thing, the y-axis variable here is the count of applications not the count of granted loans -0.5 for minor mislabeling
##Total for Q1: 2 pts
```
:::
...
hmda_ms_2024 |>
  ...
  theme_minimal()+
  coord_flip()
##TA Feedback: This graph looks good except for the same small labeling thing as before, the y variable here is the count of applications not the count of applicants! -0.5 for mislabeling
...
hmda_ms_2024  |> 
  ...
  theme_minimal()
##TA Feedback:This graph looks phenomenal other than the same small labeling mistake as before. -0.5 for labeling mistake. 
...
ggplot(data= outlier_filtered,
  ...
  labs(title = "Home Mortgage Disclosure Act Applications, Mississippi, 2024",
       ...
       y = "Count")+
  theme_minimal()
##TA Feedback: This graph looks great, remember to add a caption next time. -0.5 for no caption.
...
##TA Feedback: This graph looks amazing except for the same labeling mistake as before -0.5 for labeling mistake.
...
ggplot(aes(x = derived_race,
  ...
  theme(legend.position = "bottom",
        ...
        legend.key.size = unit(0.4, "cm")) +
  guides(fill=guide_legend(nrow=4))
##This graph looks marvelous except for the same labeling mistake as before! -0.5 for labeling mistake.
##Total for Q1-Q6: 12 pts
```
...
hmda_ms_2024 |>
  ...
  theme_minimal() 
##TA Feedback: This graph look fantastic! +2.5
...
hmda_denial_by_race |>
  ...
  theme_minimal() +
  coord_flip()
##TA Feedback: This graph looks phenomenal! +2.5 pts
...
hmda_ms_2024 |>
  ...
  theme_minimal()+
  facet_wrap(vars(derived_race))
##TA Feedback: this graph looks tremendous! +2.5
...
#Indeed, this dataset has some sensitive financial information like property value, debt-to-income ratio, loan purpose, and loan-to-value ratio. If it were possible to re-identify individual applicants, it would make individuals who took loans with a low debt-to-income ratio known to the public, which is personal financial information that should not be known. It will make individuals lose trust in banks and make people shy away from loans when in a deep financial crisis because of the fear of their problem being made public. The most vulnerable applicants would be black or African Americans because they were recorded to have the highest denials due to credit history. I really like that this type of data is shared because it creates awareness about considerations and injustice when loans are granted, and I also like the precautions taken to remove identifiable variables, which doesnt make any individual known. Even though the data discloses the state location of the individuals, which may create assumptions about the black people in the area having low credit history, no identifier was mentioned, so I think the data has a more positive social impact of creating awareness and reducing such injustice.
