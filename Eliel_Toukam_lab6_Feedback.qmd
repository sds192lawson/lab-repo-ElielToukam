...
names_to= "Measure",
values_to = "Value")
##TA Feedback: This looks good just remember you can also complete this pivot by writing 2:117 as the first argument instead of just subtracting one column. +2, -1 for 1 incorrect argument.
##Total for Q1: +2
```
...
# Uncomment below and complete the code to separate the Measure column. Store the results in pit_separate_1
 ...
 pit_separate_1 <- pit_pivoted |> separate(Measure, into = c("Shel_Unshel", "Demographic"), sep = " - ")
...
pit_separate_2 <- pit_separate_1 |> separate(Demographic, into = c("Demographic", "Year"), sep = ", ")
##TA Feedback: This looks amazing! +4
...
pit_by_race <- pit_str_cleaned |>
  ...
  "White", 
  "Multiple Races"))
##TA Feedback: This looks phenomenal! +4
##Total for Q1-Q5: 17 pts
```
...
# Step 1
gender_pivoted_longer <- gender |> pivot_longer(cols= estimate_2015: moe_2020, names_to ="Measure", values_to = "Values")
  
#step 2
gender_separated <- gender_pivoted_longer |> separate(Measure, into = c("Measure", "Year"), sep= "_")

# Step 3
gender_final <- gender_separated |> pivot_wider(names_from = "Measure", values_from = "Values")

# Step 1
race_pivoted_longer <- race |> pivot_longer(cols= estimate_2015: moe_2020, names_to ="Measure", values_to = "Values")
  
#Step 2
race_separated <- race_pivoted_longer |> separate(Measure, into = c("Measure", "Year"), sep= "_")

# Step 3
race_final <- race_separated |> pivot_wider(names_from = "Measure", values_from = "Values")
##TA Feedback: This looks wonderful! +4
##Total for Q1-Q6: 21 pts
```
...
pit_gender <- pit_by_gender |> 
  inner_join(gender_final, by= c("StateName" = "NAME", "Demographic" = "variable", "Year" = "Year"))

pit_race <- pit_by_race |>
  inner_join(race_final,by= c("StateName" = "NAME", "Demographic" = "variable", "Year" = "Year"))
##This looks marvelous! +4
##Total for Q1-Q7: 25 pts
...
pit_race_rates <- pit_race |>
  mutate(homeless_rate = Value/estimate * 10000)
##TA Feedback: This looks terrific! +4
##Total for Q1-Q8: 29 pts
#| echo: false
#| document: solutions
...
#- +2 for attempt
#- +1 for correct function (inner_join())
#- +0.5 for each correct set of arguments within inner_join()
```
...
```{r}
#I think as data scientists we should say what methods and considerations were used to collect our data so that our audience is informed about the discrepancy that may exist in the data.The PIT count data gives value and information to people on the aspect of homelessness in the US even though not all people may be counted because only those visible to enumerators are counted. I think data collected cannot be really perfect and as far as reasons for possible discrepancy is listed, it is fine.To collect this data, enumerators could open a food sharing campaign for unhoused individuals that would call people since they'll want free stuff then they would be counted.
...
##TA Grading: Final Score: 29/30 or 96.67%
