...
#| echo: false
sqf_2011 |> head()
#There is no distinct unit of observation except that 1= yes or true and 0= no or false for the variables like wpnfound that have it.
```
...
sqf_2011 <-
  sqf_2011 |>
  select(pct, arrestsumm,frisked, wpnfound, race, age)
##TA Feedback: This looks great! +3
...
total_stops
##TA Feedback: This looks amazing! +3
...
##TA Feedback: This looks awesome! +3
...
##TA Feedback: This looks fantastic! +3
##Total for Q1-Q5: 15 pts
```
...
##TA Feedback: This looks phenomenal! +3
##Total for Q1-Q6: 18 pts
```
...
arrange(desc(stops)) 
##TA Feedback: This looks wondrous! +3
##Total for Q1-Q7: 21 pts
```
...
```{r, eval = FALSE}
sqf_2011 |> 
  filter(race %in% c("P", "Q")) |> 
 summarize(stops_Latinx = n(), 
            percent_Latinx = n() / total_stops_race_recorded * 100)
##TA Feedback: This looks marvelous! +3 
##Total for Q1-Q8: 24 pts
```
...
sqf_2011 |>
  group_by(race) |>
  summarize(stops= n(),
            percent_stops= mean(stops)/total_stops_race_recorded *100,
    percent_frisked= mean(frisked)/n() *100,
percent_wpnfound= mean(wpnfound)/n() *100,
percent_arrestsumm= mean(arrestsumm)/n() *100
  )
##TA Feedback: This looks wonderful, just remember to add na.rm=TRUE to remove na values next time. +2.5
##Total for Q1-Q9: 26.5 pts
```
...
```{r, eval = FALSE}
#From the code above, I found out that that Blacks had the highest percent stops but they had the lowest percent for weapon found, arrests and frisked while the American Indian or Alaska natives had the lowest stops but were shown to have to highest percent of weapons found, frisks and arrest sums. This data confirms that the constitutionality of 2011 stop and frisk activity is NYC was racist especially against black people because many of them were stopped and were not frisked, nor arrested nor found with a weapon.
##TA Feedback: Good answer! +3
##Total for Q1-Q10: 29.5 pts
```
...
#It was interesting and at the same time disturbing that this dataset was made based from suspicion of police officers. I think they did really good by saying this directly so that viewers have a good background information about the analysis they are seeing. But suspicion only is not enough to stop and it is worst that not all officers fill the UF-250 for every stop making the process even more unethical. Measures should be taken to make this form filled so that we can anlyse which cops are being racist as seen in the analysis. If cops who made the stops were recorded, we could analyse the stops made by each cop or group of cops to detect who is racist or not so that appropriate measures can be taken to ensure racial justice as I believe that not all cops were highly suspicious black people
:::
...
```{r}
sqf_2011 <- sqf_2011 |>
  mutate(wpnfoundword= (ifelse(wpnfound== 1, "Yes","No")))

ggplot(data= sqf_2011,
  aes(x= race, fill= wpnfoundword)
)+
  geom_bar()+
  theme_minimal()+
  scale_y_continuous(labels =scales::comma)
```
#it is interesting that blacks were the highest people with weapons.Now, I don't really blame cops for suspecting them more.

##TA Grading: Final Score: 29.5/30 or 98.33%
