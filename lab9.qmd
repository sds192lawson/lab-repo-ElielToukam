...
la_map <- leaflet(width = "100%") |>
 setView(lat = 30.52, lng = -91.52, zoom = 6) |>
...
#the colors are different because the functions divide the values in X107..TOTAL.RELEASES into different sets. The Numeric function makes a sequence of numbers representing different colors. the Quantile function divides the numbers into a frequency showing where points data is located. And the bin functions divides the values into bins that represent different colors. 
:::
...
la_map |>
  addCircleMarkers(data = tri_la_2023,
                   radius = ~X107..TOTAL.RELEASES/1000000,
                   weight = 1,
                   popup = ~paste("X4..FACILITY.NAME", "X23..INDUSTRY.SECTOR","X..CHEMICAL.TEXT", sep = "_"),
...
#Getting the pollution data can seem difficult. If data about the number of products produced by companies is collected, scientists could provide an estimate of the amountpollution that can be produced from that amount of waste. This reference could be used as reference to ensure companies are being truthful. A challenge with this apporach is having companies give out their production data.To achieve this, we would need to add how much if chemical were produced in the table of data.
