...
open_payments <- open_payments |> 
  mutate(Covered_Recipient_Full=  paste(Covered_Recipient_First_Name, Covered_Recipient_Last_Name))
##TA Feedback: This looks beautiful! +3
...
open_payments |>
  select(Covered_Recipient_NPI, Covered_Recipient_Full) |>
  map_int(num_unique)
##TA Feedback: This looks perfect! +3
...
open_payments |>
  ...
  mutate(num_names = length(unique(Covered_Recipient_Full))) |>
  ungroup() |>
  filter(num_names > 1) |>
  select(Covered_Recipient_NPI, Covered_Recipient_Full) |>
  distinct() |>
...
top_10_doctors <- open_payments |>
  group_by(Covered_Recipient_Full) |>
  summarize(Total_Amount_of_Payment = sum(Total_Amount_of_Payment_USDollars),
            Covered_Recipient_NPI= first(Covered_Recipient_NPI)) |>
   arrange(desc(Total_Amount_of_Payment)) |>
  head(10)
...
top_10_doctors_ids <- top_10_doctors |> pull(Covered_Recipient_NPI) 

print(top_10_doctors_ids)
...
calculate_payment_type_amts <- function(doctor_id){
 ...
 top_10_doctors_ids |>  map(calculate_payment_type_amts) |> set_names(top_10_doctors_names)
...
payments_calendar <- function(doctor_id, doctor_name){ 
  open_payments |>
    filter(Covered_Recipient_NPI == doctor_id) |>
    ##TA Feedback: You need to summarize here first: summarize(specialty = first(Covered_Recipient_Specialty_1)) |>
    #pull()
    ...
    labs(title = doctor_name, 
         subtitle = "Covered_Recipient_Specialty_1",
         y = "", 
         x = "Day",
         fill = "Payment Amount") +
...
##TA Feedback: Otherwise this looks awesome! +2 
...
calculate_manufacturer_payments <- function(doctor_id) {
  open_payments |>
    ...
    summarize(Total_Amount_Payments = sum(Total_Amount_of_Payment_USDollars),
       doctors_name = first(Covered_Recipient_Full)) |>
    arrange(desc(Total_Amount_Payments))
}
##TA Feedback: Don't forget your map function: 
#TA Feedbackmap_df(top_10_doctors_ids, calculate_manufacturer_payments) |> <--Code you forgot!
```

```{r}
# Plot resulting data frame here
ggplot(data = Manufacturer_table, 
       aes(y= Applicable_Manufacturer_or_Applicable_GPO_Making_Payment_Name, x= Total_Amount_Payments, fill = Form_of_Payment_or_Transfer_of_Value))+
  geom_col()+
  theme_minimal()+
  facet_wrap(vars(doctors_name), ncol= 2)
##TA Feedback: Otherwise this looks beautiful! +2
##Total for Q1-Q10: 28 pts
```
...
```{r}
drug_and_devices <- function(doctor_id) {
  open_payments |>
    filter(Covered_Recipient_NPI == doctor_id) |>
    group_by() |>
    summarize(Total_Amount_Payments = sum(Total_Amount_of_Payment_USDollars,
            Total_Payment = sum(Form_of_Payment_or_Transfer_of_Value)))
}

 drug_table <- top_10_doctors_ids |> map(drug_and_devices)
...
#It is definite that the data set has alot of information that can impact how citizens choose their healthcare but was the dataset created for that purpose? I think if the purpose was to educate people then the law was right. I say this because the dataset has other benefits to the state. Besides, it is always good to be informed, so eitherway I think it was a good law even though it looks forced. Some drawbacks that this policy might have is it may prevent people from going for good health services because of the prices seen. It is also important to not that a hospital is a business and such a dataset may expose the way they make gains.

:::

##TA Grading: Final Score: 28/30 pts or 93.33%
