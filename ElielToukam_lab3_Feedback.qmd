...
nbi_hampshire |>
  ...
  theme_minimal()
##TA Feedback: Your code looks awesome nice job +3
##Q1 total: 3 pts
...
nbi_hampshire |>
  ...
  labs(title = "Highway Bridge Conditions, Hampshire County, MA, 2024",
       ...
       colour = "Road") +
  theme_minimal()
##TA Feedback: This looks nice, remember you can use 'col=' to specify dot color in dot plots, instead of having to write out colour each time +3
##Total Q1-Q2: 6 pts
```
...
nbi_hampshire |>
  ggplot(aes(x = YEAR_BUILT_027, 
             y = STRUCTURAL_EVAL_067,
             colour = STRUCTURAL_EVAL_067)) +
  geom_point(size = 2, alpha = 0.5, position = "jitter") +
  labs(title = "Highway Bridge Conditions, Hampshire County, MA, 2024",
       x = "Year Built", 
       y = "Structural Evaluation",
       colour = "Road Structure") +
  theme_minimal()

# I noticed that in the first code the colors were completely different showing different categories of road but this second code, the colors were same but different gradient showing that its same structural categrory of data but arranged sequentially which is understood per amount of a certain material.

##TA Feedback: Your graph looks awesome, +3
...
nbi_hampshire |>
  ...
  theme_minimal()
##TA Feedback: Try not putting the column name you are using to determine the dot shape in quotes, then your graph should have multiple shapes. -1 for incorrect code
##Total for Q1-Q4: 11 pts
```

##Here's the code for the correct graph, let me know if you have any further questions
...
#I added the size to the aes function hence plotting cost of building bridges pf hampshire county. From the plot, I noticed that the most expensive bridges were built in the years between 1950 and 2000 but it is interesting to see that those bridges have a lower structural evaluation 

##TA Feedback: This looks great I just have one small note about your interpretation, and labeling: BRIDGE_IMP_COST_094 is the cost of improving the bridge, so its more likely that the parts or construction methods used to improve bridges originally built between 1950 and 2000 were the costliest not the bridge themselves. -0.5 for slightly incorrect label
##Total for Q1-Q5: 13.5 pts
...
##TA Feedback: This graph looks great, nice job on the labels! +3
##Total for Q1-Q6: 16.5 pts
...
nbi_ma |>
  ...
  labs(title = "Highway Bridge Conditions, MA, 2024",
       ...
       y = "Route Prefix") +
  theme_minimal()+
  scale_x_binned(breaks= c( 1954, 2021), labels = c("when Eisenhower was elected", "Eisenhower elected"))

##TA Feedback: Remember the second label, for after 2021 should be "Today" not "Eisenhower Elected" which should be the first label. -0.5 for incorrect and missing labels in scale_x_binned
##Total for Q1-Q7: 19 pts

##Correct labels: scale_x_binned(breaks = c(1954, 2021), labels = c("Eisenhower Elected", "Today"))
```
...
#To ensure accountability of bridge inspections, the process of inspecting certain group of bridges can be divided per group of persons and also the process be divided into steps that are clear and straight forward. The steps of maintenance will assist with clear instructions. Additionally, research can be done on the degradation of the bridges and graphs plotted to show how this might impact society which each step of maintenance highlighted at the side to prevent such consequences can encourage people to be more careful with bridge maintenace. A policy can be put in place that bridges should be properly checked following steps every year or so.

##TA Grading: Final score: 28/30 or 93.33%
